...
- [ ] `\cite{}` key 非空
- [ ] `\ref{}`、`\label{}` 命名规范
- [ ] 新增或改动的 `\label{}` 在全文唯一，不与既有 label 重复
- [ ] 新增或改动的 `\ref{}`、`\autoref{}`、`\pageref{}` 均指向真实存在的 label
- [ ] 新增或改动的 `\cite{}` key 已存在于目标 `refs.bib`，且不得依赖 `\nocite{*}` 把未引用文献强行塞入文末
- [ ] 图、表、公式、算法等浮动体均在正文中有解释性引用，而不是只有 `\label{}`
- [ ] 中文正文没有混入未解释的英文占位符
- [ ] 不新增未经确认的宏包或环境
...
- [ ] 新增内容不偏离原段落目标。
- [ ] 引用不堆砌、不虚构。
- [ ] 每条新增参考文献都有 DOI、URL、ISBN、标准号或权威数据库记录，可被独立检索。
- [ ] 技术前沿、课程大作业、AI/机器学习、产品数据类引用默认在近 3 年内；超过 3 年必须说明“经典/标准/历史基线/无新版替代”的保留理由。
- [ ] 文后参考文献不得出现正文未引用的孤儿条目；若主文件存在 `\nocite{*}`，审稿人必须要求移除或逐条解释。
- [ ] 读起来像论文正文，而不是 AI 总结。
...
- [ ] 插入段落出现在正确章节
- [ ] 图表、目录、页眉页脚没有异常
- [ ] 渲染 PDF 后逐页检查，确认新增表格/图片/公式没有造成上一页大面积留白；若某页有效内容低于约 40% 且非封面、目录、章节首页或附录起始页，必须调整浮动体或拆分内容
