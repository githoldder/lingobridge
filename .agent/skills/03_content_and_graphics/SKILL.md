...
本模块涵盖论文逻辑框架搭建、基于 AI 的内容扩写，以及使用 Figma/Python 进行学术图表绘制的工程化流水线，旨在降低模板化表达、逻辑支撑不足和排版不一致等风险。
...
严禁让 AI 在缺乏资料的情况下直接扩写，必须通过**“补充输入 + 风格锚定”**来保证信息密度。每次进行单一段落扩写时，**必须**完整附带以下 Prompt，以提升文字连贯性并避免模板化分点。
...
> 5. **排版格式**：**内容不可分点展开**，但需以明确的二级/三级标题区分。每个小标题下的内容应详尽全面，避免仅写简单的短句或大纲。
...
## 3. 学术图表生成
学术论文要求图表具备高清晰度和统一审美，原则上不直接使用未经处理的截图。
