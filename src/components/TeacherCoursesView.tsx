...
const TeacherCoursesView: React.FC<TeacherCoursesViewProps> = ({ onNavigate }) => {
  ...
  const [title, setTitle] = useState('第三课：自我介绍');
  ...
  const loadCourses = async () => {
    ...
    try {
      ...
      setMessage(error.message || 'Failed to load courses. Start backend on port 3001.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const createCourse = async () => {
    setMessage('');
    try {
      const course = await coursesApi.create(title || 'New Chinese Course');
      setCourses((prev) => [course, ...prev]);
      setSelectedCourseId(course.id);
      setMessage('Course created.');
    } catch (error: any) {
      setMessage(error.message || 'Create course failed.');
    }
  };
  ...
  const uploadFile = async (file?: File) => {
    ...
    try {
      ...
      const exercises = result.exercises?.length || 0;
      setMessage(`${file.name} uploaded. Pages: ${pages}, exercises: ${exercises}.`);
      await loadCourses();
    } catch (error: any) {
    ...
    <div id="teacher-courses" className="space-y-8">
      <div className="flex flex-col lg:flex-row justify-between gap-6 lg:items-end">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-1">Upload PPT/PDF/Excel and prepare the MVP learning loop.</p>
        </div>
      ...
      <div className="bg-blue-50 border border-blue-100 text-[#0056D2] rounded-2xl px-5 py-4 text-sm font-bold flex items-center gap-3">
        ...
        {message || 'Select a course, then upload .pptx/.pdf for pages or .xlsx for practice exercises.'}
      </div>

      {isLoading ? (
        <div className="py-20 text-center text-gray-400 font-bold">Loading courses...</div>
      ) : (
          ...
          {courses.map((course) => (
            ...
            >
              ...
              <div className="p-5 flex-1 flex flex-col">
                ...
                <p className="text-xs text-gray-400 font-medium mb-4 line-clamp-2">{course.description || 'MVP course'}</p>
                ...
                <div className="flex gap-2 mt-auto">
                  <button className="flex-1 py-2 bg-gray-50 text-gray-600 rounded-lg text-xs font-bold border border-gray-100">
                    {course.exercisesCount || 0} Exercises
                  </button>
                  <button
                    onClick={(event) => {
